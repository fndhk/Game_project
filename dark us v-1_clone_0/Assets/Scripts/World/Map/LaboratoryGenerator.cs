...
namespace ArtNotes.UndergroundLaboratoryGenerator
{
    ...
    {
        [Serializable]
        public class ItemSpawnBalanceRule
        {
            public ItemType TargetItemType = global::ItemType.Camera;
        ...
        private class ItemSpawnTypeOption
        {
            ...
            public bool CanSpawnMore => Prefabs.Count > 0 && SpawnedCount < Mathf.Max(0, MaxCount);
        ...
        [Header("Item Spawn Balance")]
        [Tooltip("켜면 WorldItemPickup.itemType 기준으로 아이템 종류별 스폰 비율/상한을 적용")]
        public bool UseItemTypeSpawnBalance = true;

        [Tooltip("카메라는 많이, 메디킷은 적게, 칼은 제한되도록 기본값을 둠")]
        public ItemSpawnBalanceRule[] ItemSpawnBalanceRules = new ItemSpawnBalanceRule[]
        {
            new ItemSpawnBalanceRule { TargetItemType = global::ItemType.Camera, SpawnWeight = 6, MinCount = 6, MaxCount = 100 },
            new ItemSpawnBalanceRule { TargetItemType = global::ItemType.Medkit, SpawnWeight = 2, MinCount = 1, MaxCount = 4 },
            new ItemSpawnBalanceRule { TargetItemType = global::ItemType.Knife, SpawnWeight = 1, MinCount = 0, MaxCount = 2 },
            new ItemSpawnBalanceRule { TargetItemType = global::ItemType.ScannerBattery, SpawnWeight = 2, MinCount = 0, MaxCount = 3 }
        };
        ...
        private IEnumerator Start()
        {
            ...
            else
            {
                ...
                global::GameplayStartupGate.SetMapGenerationBlocked(false);
        ...
        private void SpawnItemsAfterGeneratedMap()
        {
            ...
            List<GameObject> itemSpawnPlan = BuildItemSpawnPlan(validItemPrefabs);

            if (itemSpawnPlan.Count <= 0)
            {
                Debug.LogWarning("[LaboratoryGenerator] 아이템 스폰 밸런스 결과가 비어 있어서 아이템을 생성하지 않음.");
                return;
            }
            ...
            for (int i = 0; i < itemSpawnPlan.Count; i++)
            {
                ...
                GameObject itemPrefab = itemSpawnPlan[i];
        ...
        // 아이템 종류별 가중치/최소/최대 개수를 반영한 스폰 계획을 만든다.
        private List<GameObject> BuildItemSpawnPlan(List<GameObject> validItemPrefabs)
        {
            List<GameObject> plan = new List<GameObject>();
            int targetCount = Mathf.Max(0, ItemSpawnCount);

            if (targetCount <= 0 || validItemPrefabs == null || validItemPrefabs.Count <= 0)
            {
                return plan;
            }

            if (!UseItemTypeSpawnBalance)
            {
                for (int i = 0; i < targetCount; i++)
                {
                    plan.Add(validItemPrefabs[Random.Range(0, validItemPrefabs.Count)]);
                }

                return plan;
            }

            List<ItemSpawnTypeOption> options = BuildItemSpawnTypeOptions(validItemPrefabs);
            ...
            for (int i = 0; i < options.Count && plan.Count < targetCount; i++)
            {
                ItemSpawnTypeOption option = options[i];
                int requiredCount = Mathf.Max(0, option.MinCount);

                while (plan.Count < targetCount && option.SpawnedCount < requiredCount && option.CanSpawnMore)
                {
                    AddItemToSpawnPlan(plan, option);
                }
            }

            while (plan.Count < targetCount)
            {
                ItemSpawnTypeOption selectedOption = SelectWeightedItemSpawnOption(options);
                ...
                AddItemToSpawnPlan(plan, selectedOption);
            }

            ShuffleItemSpawnPlan(plan);
            return plan;
        }

        private List<ItemSpawnTypeOption> BuildItemSpawnTypeOptions(List<GameObject> validItemPrefabs)
        {
            List<ItemSpawnTypeOption> options = new List<ItemSpawnTypeOption>();

            for (int i = 0; i < validItemPrefabs.Count; i++)
            {
                GameObject prefab = validItemPrefabs[i];
                ...
                ItemType itemType = GetPrefabItemType(prefab);
                ItemSpawnTypeOption option = FindItemSpawnTypeOption(options, itemType);

                if (option == null)
                {
                    ItemSpawnBalanceRule rule = FindItemSpawnBalanceRule(itemType);
                    option = new ItemSpawnTypeOption
                    {
                        TargetItemType = itemType,
                        SpawnWeight = rule != null ? Mathf.Max(0, rule.SpawnWeight) : 1,
                        MinCount = rule != null ? Mathf.Max(0, rule.MinCount) : 0,
                        MaxCount = rule != null ? Mathf.Max(0, rule.MaxCount) : 100
                    };
                    options.Add(option);
                }

                option.Prefabs.Add(prefab);
            }

            return options;
        }

        private ItemType GetPrefabItemType(GameObject prefab)
        {
            if (prefab == null)
            {
            ...
            return pickup != null ? pickup.itemType : global::ItemType.None;
        }

        private ItemSpawnTypeOption FindItemSpawnTypeOption(List<ItemSpawnTypeOption> options, ItemType itemType)
        {
            for (int i = 0; i < options.Count; i++)
            {
                if (options[i] != null && options[i].TargetItemType == itemType)
                {
                    return options[i];
                }
            }
        ...
        private ItemSpawnBalanceRule FindItemSpawnBalanceRule(ItemType itemType)
        {
            if (ItemSpawnBalanceRules == null)
            {
                return null;
            }

            for (int i = 0; i < ItemSpawnBalanceRules.Length; i++)
            {
                ItemSpawnBalanceRule rule = ItemSpawnBalanceRules[i];

                if (rule != null && rule.TargetItemType == itemType)
                {
                    return rule;
                }
            }
        ...
        private ItemSpawnTypeOption SelectWeightedItemSpawnOption(List<ItemSpawnTypeOption> options)
        {
            int totalWeight = 0;
            ...
            for (int i = 0; i < options.Count; i++)
            {
                ...
                if (option != null && option.CanSpawnMore)
                {
                    totalWeight += Mathf.Max(0, option.SpawnWeight);
                }
            }
            ...
            if (totalWeight <= 0)
            {
                for (int i = 0; i < options.Count; i++)
                {
                    if (options[i] != null && options[i].CanSpawnMore)
                    {
                        return options[i];
                    }
                }
            ...
            for (int i = 0; i < options.Count; i++)
            {
                ...
                currentWeight += Mathf.Max(0, option.SpawnWeight);
        ...
        private void AddItemToSpawnPlan(List<GameObject> plan, ItemSpawnTypeOption option)
        {
            if (plan == null || option == null || !option.CanSpawnMore)
            {
                return;
            }

            GameObject prefab = option.GetRandomPrefab();
        ...
        private void ShuffleItemSpawnPlan(List<GameObject> plan)
        {
            if (plan == null)
            {
                return;
            ...
            {
                int swapIndex = Random.Range(i, plan.Count);
                GameObject temp = plan[i];
