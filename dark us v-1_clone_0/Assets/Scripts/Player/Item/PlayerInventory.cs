...
public class PlayerInventory : MonoBehaviour
{
    ...
    [Header("Stack Limit")]
    // 같은 아이템은 슬롯 하나에 최대 이 개수까지만 보관한다.
    ...
    {
        maxAmountPerItem = Mathf.Max(1, maxAmountPerItem);
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            ...
            {
                slots[i].amount = Mathf.Min(slots[i].amount, maxAmountPerItem);
            }
        }
    ...
    public int GetItemAmount(ItemType itemType)
    {
        ...
        return Mathf.Min(totalAmount, maxAmountPerItem);
    }

    // 특정 아이템을 더 들 수 있는지 반환한다.
    public bool CanAddItem(ItemType itemType)
    {
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            if (slots[i].itemType == itemType && slots[i].amount > 0)
            {
                return slots[i].amount < maxAmountPerItem;
            }
        }
    ...
    {
        int addedAmount;
        return TryAddItem(itemType, amount, out addedAmount);
    }

    // 아이템을 가능한 만큼 인벤토리에 넣고 실제로 들어간 개수를 반환한다.
    public bool TryAddItem(ItemType itemType, int amount, out int addedAmount)
    {
        ValidateSlots();
        addedAmount = 0;
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            if (slots[i].itemType == itemType && slots[i].amount > 0)
            {
                int addableAmount = Mathf.Min(amount, maxAmountPerItem - slots[i].amount);
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            if (slots[i].itemType == ItemType.None || slots[i].amount <= 0)
            {
                ...
                slots[i].amount = Mathf.Min(amount, maxAmountPerItem);
                addedAmount = slots[i].amount;
                RefreshSlotUi();
                return true;
            }
        }
