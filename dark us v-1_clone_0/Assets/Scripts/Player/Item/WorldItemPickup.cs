...
public class WorldItemPickup : MonoBehaviour, IPlayerInteractable
{
    ...
    // 마지막 상호작용에서 실제로 아이템이 들어갔는지 저장한다.
    public bool LastInteractSucceeded { get; private set; }

    // 마지막 상호작용에서 실제로 들어간 개수이다.
    ...
    public void Interact(PlayerObjectiveInteractor interactor)
    {
        ...
        if (inventory == null)
        {
            ...
            ShowHudNotice(interactor, InGameLocalization.Text("Inventory is full."), true);
            return;
        }

        int requestedAmount = Mathf.Max(1, amount);
        bool added = inventory.TryAddItem(itemType, requestedAmount, out int addedAmount);

        if (!added)
        {
            Debug.Log(InGameLocalization.Text("Inventory is full."));
            ShowHudNotice(interactor, GetCannotCarryMessage(inventory), true);
            return;
        }

        LastInteractSucceeded = true;
        LastAddedAmount = addedAmount;
        ShowHudNotice(interactor, GetItemDisplayName() + " x" + inventory.GetItemAmount(itemType), false);

        if (addedAmount < requestedAmount)
        {
            amount = requestedAmount - addedAmount;
            return;
        }
    ...
    private string GetCannotCarryMessage(PlayerInventory inventory)
    {
        string itemName = GetItemDisplayName();

        if (inventory != null && inventory.GetItemAmount(itemType) >= inventory.maxAmountPerItem)
        {
            return InGameLocalization.Text("Cannot carry more") + " " + itemName + " x" + inventory.maxAmountPerItem;
        }

        return InGameLocalization.Text("Inventory is full.");
    }

    private void ShowHudNotice(PlayerObjectiveInteractor interactor, string message, bool warning)
    {
        ...
        PlayerHUDController hud = interactor.GetComponent<PlayerHUDController>();

        if (hud == null)
        {
            hud = interactor.GetComponentInParent<PlayerHUDController>();
        }

        if (hud != null)
        {
            hud.ShowItemNotice(message, warning);
        }
    }
    ...
    private void RemoveExistingScanDots()
    {
        ...
        {
            scanDotRenderers = Object.FindObjectsByType<InstancedScanDotRenderer>(FindObjectsInactive.Exclude);
        }
