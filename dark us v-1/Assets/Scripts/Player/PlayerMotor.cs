...
// 마우스 시점 회전은 기존 MouseLook 스크립트를 그대로 사용하는 것을 기준으로 한다.
[RequireComponent(typeof(CharacterController))]
public class PlayerMotor : MonoBehaviour
{
    [Header("참조")]
    // 플레이어의 눈 역할을 하는 카메라 Transform이다.
    // 보통 Main Camera를 여기에 넣으면 된다.
    public Transform playerCamera;

    [Header("이동 속도")]
    // 기본 이동 속도이다.
    // 사람이 천천히 걷는 느낌으로 설정했다.
    public float walkSpeed = 1.8f;

    // Shift를 누르고 앞으로 갈 때의 속도이다.
    // 너무 빠르지 않은 살짝 달리는 느낌으로 설정했다.
    public float sprintSpeed = 2.7f;

    // Ctrl을 누르고 있을 때의 속도이다.
    // 웅크린 상태답게 매우 천천히 움직이도록 설정했다.
    ...
    [Header("점프 / 중력")]
    // 기본 걷기 상태에서의 점프 높이이다.
    // 너무 높지 않게 자연스러운 높이로 설정했다.
    public float walkJumpHeight = 0.9f;

    // 달리기 상태에서의 점프 높이이다.
    // 기본 점프보다 아주 약간 더 높게 설정했다.
    ...
    // 현재 CharacterController가 바닥에 닿아 있는지 갱신하는 함수이다.
    private void UpdateGroundedState()
    {
        // 유니티의 isGrounded 값을 그대로 저장한다.
    ...
    private float GetCurrentJumpHeight()
    {
        ...
        // 달리기 상태면 기본보다 조금 더 높은 점프를 사용한다.
        if (IsTryingToSprint())
        {
            return sprintJumpHeight;
        }

        // 그 외에는 기본 걷기 점프를 사용한다.
        return walkJumpHeight;
    }

    // 지금 달리기 조건인지 확인하는 함수이다.
    private bool IsTryingToSprint()
    {
        // 앞으로 이동 입력을 받고 있고 Shift를 누르고 있을 때만 달리기로 본다.
    ...
    private void HandleHorizontalMovement()
    {
        ...
        {
            // 웅크리지 않았고 Shift 달리기 조건이면 달리기 속도를 사용한다.
            if (IsTryingToSprint())
            {
