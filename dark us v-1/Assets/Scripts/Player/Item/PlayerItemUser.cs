...
public class PlayerItemUser : MonoBehaviour
{
    ...
    private string itemFailureNotice;
    ...
    private void AutoFindReferences()
    {
        ...
        if (dotRenderer == null)
        {
            dotRenderer = Object.FindAnyObjectByType<InstancedScanDotRenderer>();
        }
        ...
        if (hudController == null)
        {
            hudController = GetComponentInParent<PlayerHUDController>();
        }
    }
    ...
    public void DropSelectedItem()
    {
        ...
        if (!inventory.TryRemoveSelectedItem(1, out ItemType droppedItemType, out int droppedAmount))
        {
            ...
            ShowItemNotice(InGameLocalization.Text("No item selected."), true);
            return;
        }

        SpawnDroppedItem(droppedItemType, droppedAmount);
        GameAudioManager.PlayItemDrop();
        ShowItemNotice(GetUsedOrDroppedMessage("Dropped", droppedItemType), false);
    }
    ...
    public void UseSelectedItem()
    {
        ...
        if (shouldConsume)
        {
            ...
            ShowItemNotice(GetUsedOrDroppedMessage("Used", selectedItem), false);
        }
        else
        {
            ShowItemNotice(string.IsNullOrWhiteSpace(itemFailureNotice) ? InGameLocalization.Text("Cannot use item") : itemFailureNotice, true);
        }
    }
    ...
    private bool TryUseCameraItem()
    {
        if (playerCamera == null || dotRenderer == null)
        {
            ...
            itemFailureNotice = InGameLocalization.Text("Cannot use item");
            return false;
        }
    ...
    private bool TryUseMedkitItem()
    {
        ...
        if (playerStats.currentHealth >= playerStats.maxHealth)
        {
            ...
            itemFailureNotice = InGameLocalization.Text("Health is already full.");
            return false;
        }
    ...
    private string GetUsedOrDroppedMessage(string actionKey, ItemType itemType)
    {
        int remainingAmount = inventory != null ? inventory.GetItemAmount(itemType) : 0;
        string message = InGameLocalization.Text(actionKey) + " " + InGameLocalization.ItemName(itemType);

        if (remainingAmount > 0)
        {
            message += " x" + remainingAmount;
        }

        return message;
    }

    private void ShowItemNotice(string message, bool warning)
    {
        if (hudController == null)
        {
            AutoFindReferences();
        ...
        if (hudController != null)
        {
            hudController.ShowItemNotice(message, warning);
        }
    }
