...
public class PlayerHUDController : MonoBehaviour
{
    ...
    public float objectiveProgressBarWidth = 250f;

    [Header("Item Feedback")]
    public float itemNoticeDuration = 2.2f;
    public float itemNoticeFadeDuration = 0.35f;
    ...
    private TMP_Text objectiveDetailText;
    private Image objectiveProgressFill;
    private TMP_Text roundStatusText;
    private RectTransform itemNoticeRoot;
    private CanvasGroup itemNoticeCanvasGroup;
    private TMP_Text itemNoticeText;
    ...
    private float itemNoticeVisibleUntil;
    private Color itemNoticeTextColor = Color.white;
    ...
    private void BuildItemNoticeModule()
    {
        itemNoticeRoot = CreateRect("ItemNotice", hudRoot, new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0f, 118f));
        itemNoticeRoot.sizeDelta = new Vector2(430f, 38f);
        AddPanel(itemNoticeRoot, new Color(0f, 0f, 0f, 0.72f));
        AddOutline(itemNoticeRoot, new Color(0.78f, 0.88f, 0.84f, 0.34f), new Vector2(1f, -1f));
        itemNoticeCanvasGroup = itemNoticeRoot.gameObject.AddComponent<CanvasGroup>();

        itemNoticeText = CreateLabel("", itemNoticeRoot, Vector2.zero, Vector2.one, new Vector2(0.5f, 0.5f), Vector2.zero, 16, lineColor, TextAlignmentOptions.Center);
        itemNoticeText.rectTransform.offsetMin = new Vector2(14f, 4f);
        itemNoticeText.rectTransform.offsetMax = new Vector2(-14f, -4f);
        itemNoticeText.textWrappingMode = TextWrappingModes.NoWrap;
        itemNoticeRoot.gameObject.SetActive(false);
    }
    ...
    private void BuildObjectiveModule()
    {
        ...
        objectiveRuntimeText.rectTransform.sizeDelta = new Vector2(520f, 30f);

        objectiveDetailText = CreateLabel("", hudRoot, new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(-54f, -78f), 13, new Color(0.72f, 0.86f, 0.88f, 0.82f), TextAlignmentOptions.Right);
        objectiveDetailText.rectTransform.sizeDelta = new Vector2(520f, 24f);

        RectTransform progressRoot = CreateRect("ObjectiveProgress", hudRoot, new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(-54f, -108f));
        progressRoot.sizeDelta = new Vector2(Mathf.Max(80f, objectiveProgressBarWidth), 6f);
        AddImage(progressRoot, new Color(0.36f, 0.44f, 0.43f, 0.34f));

        RectTransform progressFillRect = CreateRect("Fill", progressRoot, Vector2.zero, new Vector2(0f, 1f), new Vector2(0f, 0.5f), Vector2.zero);
        progressFillRect.offsetMin = Vector2.zero;
        progressFillRect.offsetMax = Vector2.zero;
        objectiveProgressFill = AddImage(progressFillRect, cyanColor);
    }

    private void BuildRoundStatusModule()
    {
        roundStatusText = CreateLabel("", hudRoot, new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(-54f, -126f), 14, new Color(0.72f, 0.86f, 0.88f, 0.82f), TextAlignmentOptions.Right);
    ...
    {
        micStatusRoot = CreateRect("MicStatus", hudRoot, new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0f, 172f));
    ...
    private void UpdateObjectiveText()
    {
        ...
        else if (role == PlayerRole.Killer)
        {
            ...
            objectiveRuntimeText.text = T("Objective Kill Crew");

            if (objectiveDetailText != null)
            {
                objectiveDetailText.text = T("Alive Citizens") + " " + aliveCitizens + "/" + safeTotal;
            }

            SetObjectiveProgress(1f - Mathf.Clamp01(aliveCitizens / (float)safeTotal), new Color(1f, 0.36f, 0.28f, 0.95f));
        }
        else if (LabObjectiveManager.Instance != null)
        {
            objectiveRuntimeText.text = LabObjectiveManager.Instance.GetHudObjectiveText();
        ...
        else
        {
            ...
            if (objectiveDetailText != null)
            {
                objectiveDetailText.text = T("Progress") + " 0%";
            }

            SetObjectiveProgress(0f, cyanColor);
        }
    }

    private void SetObjectiveProgress(float normalized, Color fillColor)
    {
        ...
        RectTransform fillRect = objectiveProgressFill.rectTransform;
        fillRect.anchorMax = new Vector2(Mathf.Clamp01(normalized), 1f);
        fillRect.offsetMin = Vector2.zero;
        fillRect.offsetMax = Vector2.zero;
        objectiveProgressFill.color = fillColor;
    }

    public void ShowItemNotice(string message, bool warning)
    {
        ...
        if (itemNoticeText == null || itemNoticeRoot == null)
        {
            return;
        }

        itemNoticeTextColor = warning ? new Color(1f, 0.74f, 0.18f, 0.98f) : lineColor;
        itemNoticeText.text = message;
        itemNoticeText.color = itemNoticeTextColor;
        itemNoticeVisibleUntil = Time.unscaledTime + Mathf.Max(0.2f, itemNoticeDuration);
        itemNoticeRoot.gameObject.SetActive(true);

        if (itemNoticeCanvasGroup != null)
        {
            itemNoticeCanvasGroup.alpha = 1f;
        }
    }

    private void UpdateItemNotice()
    {
        if (itemNoticeRoot == null || itemNoticeCanvasGroup == null)
        {
            return;
        }
        ...
        float remaining = itemNoticeVisibleUntil - Time.unscaledTime;
        ...
        itemNoticeRoot.gameObject.SetActive(true);
        float fadeDuration = Mathf.Max(0.01f, itemNoticeFadeDuration);
        itemNoticeCanvasGroup.alpha = Mathf.Clamp01(remaining / fadeDuration);
    ...
    private void UpdateVoiceStatus()
    {
        ...
        if (!GameplayStartupGate.IsBlocked && Input.GetKeyDown(GameInputBindings.MicMute))
        {
            micStatusVisibleUntil = Time.unscaledTime + 2.2f;
        }
